    ...
    def get_position_shift(self, size: Size2DTypes) -> tuple[float, float]:
        ...
        return (
            (
                0.0
                if x_align == 0
                else -size.width / 2 if x_align == 1 else -size.width  # left
            ),  # h center  # right
            (
                0.0
                if y_align == 0
                else -size.height / 2 if y_align == 1 else -size.height  # top
            ),  # v center
        )  # bottom
...
__all__ = ["Anchor2D", "Anchor2DLiterals", "Anchor2DTypes"]
