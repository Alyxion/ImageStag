...
class Font:
    """
    SDK independent font handle for rendering text on images and canvases.
    """
    ...
    def __init__(
        ...
        size: int,
        framework: ImsFramework = ImsFramework.PIL,
        index: int = 0,
    ):
        """
        Initialize a font from a source.

        :param source: The font source. Either a file path, a bytes like object
            or a stream
        :param size: The font's size in points
        :param framework: The framework to be used (currently only PIL)
        :param index: The font face index (for fonts with multiple faces)
        """
    ...
    def get_y_offset(self, vert_alignment: VTextAlignmentTypes) -> int:
        """
