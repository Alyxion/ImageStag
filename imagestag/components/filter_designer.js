...
export default {
        ...
        <div class="filter-designer">
            ...
            <div class="fd-canvas-container">
                <div class="fd-toolbar">
                    ...
                    <button class="fd-toolbar-btn" @click="requestExport" title="Export pipeline">💾</button>
                    <button class="fd-toolbar-btn" @click="triggerImport" title="Import pipeline">📂</button>
    ...
    data() {
        return {
            ...
            loadingPreset: false,  // Suppress events during preset loading
        };
    },
    ...
    methods: {
        ...
        async initDrawflow() {
            ...
            this.editor.on('nodeRemoved', (id) => {
                ...
                // Emit incremental event (Python handles state update)
            ...
            this.editor.on('connectionCreated', (info) => {
                // Suppress during preset loading
                if (this.loadingPreset) return;
                // Convert Drawflow connection info to our format
                // info has: output_id, input_id, output_class (output_N), input_class (input_N)
                const fromOutput = parseInt(info.output_class.replace('output_', '')) - 1;
                const toInput = parseInt(info.input_class.replace('input_', '')) - 1;
                this.$emit('connection-added', {
                    fromNode: info.output_id,
                    toNode: info.input_id,
            ...
            // Emit layout change when node is moved
            this.editor.on('nodeMoved', (id) => {
                const dfData = this.editor.getNodeFromId(id);
                if (dfData) {
        ...
        addSourceNode(x, y, imageName) {
            ...
            // Emit incremental node-added event (unless loading preset)
        ...
        updateParam(paramName, value) {
            ...
            if (param) {
                ...
                // Debounce param-changed events to avoid overwhelming the server
                ...
                this._updateDebounceTimer = setTimeout(() => {
                    ...
                    // Emit incremental param-changed (Python handles graph update)
        ...
        addUploadedImage(imageName) {
            ...
            if (this.selectedNode && this.selectedNode.type === 'source') {
                ...
                // Emit param change for source image
                this.$emit('param-changed', {
                    nodeId: this.selectedNodeId,
                    param: 'image',
                    value: imageName
        ...
        clearGraph() {
            if (this.editor) {
                ...
                // Re-add default nodes (they emit node-added events)
                if (this.showSourceNode) {
                    this.addSourceNode(100, 200);
                }
                if (this.showOutputNode) {
        ...
        executeGraph() {
            // Python has the graph state, just signal execute
            this.$emit('execute', {});
        },
        ...
        getExportData() {
            // Get graph data in new minimal format with positions
            const graphData = this.getGraphData();
            const exportData = this.editor?.export();

            // Generate descriptive node names and track ID->name mapping
            const idToName = {};
            ...
            for (const [nodeId, node] of Object.entries(graphData.nodes)) {
                ...
                if (node.type === 'source') {
                    ...
                    baseName = 'output';
                } else if (node.filterName) {
                    // Convert CamelCase to snake_case for readability
                    baseName = node.filterName
                        .replace(/([A-Z])/g, '_$1')
                        .toLowerCase()
                        .replace(/^_/, '');
                } else {
                ...
                idToName[nodeId] = generateUniqueName(baseName);
            }

            // Second pass: build nodes with editor sub-attribute for layout
            const nodes = {};

            for (const [nodeId, node] of Object.entries(graphData.nodes)) {
                const nodeName = idToName[nodeId];

                // Get position from Drawflow export
                const dfNode = exportData?.drawflow?.Home?.data?.[nodeId];
                const x = dfNode?.pos_x;
                const y = dfNode?.pos_y;

                // Build minimal node in new format
                let minimalNode;
                if (node.type === 'source') {
                    // Export as class Source - sample image is only for presets
                    minimalNode = { class: "Source" };
                } else if (node.type === 'output') {
                    minimalNode = { class: "Output" };
                } else if (node.filterName) {
                    minimalNode = { class: node.filterName };
                    // Convert params array to dict
                    if (node.params && node.params.length > 0) {
                        minimalNode.params = {};
                        for (const param of node.params) {
                            minimalNode.params[param.name] = param.value;
                        }
                    }
                } else {
                    minimalNode = {};
                ...
                // Add layout in editor sub-attribute to avoid conflicts with filter params
                if (x !== undefined && y !== undefined) {
                    minimalNode.editor = { x, y };
                }

                nodes[nodeName] = minimalNode;
            }

            // Third pass: update connections with new names and format
            const connections = graphData.connections.map(conn => {
                const fromNodeName = idToName[conn.from_node] || conn.from_node;
                const toNodeName = idToName[conn.to_node] || conn.to_node;
                const fromPort = conn.from_port_name || 'output';
                const toPort = conn.to_port_name || 'input';

                // Build compact format: {"from": "node" or ["node", "port"], "to": ...}
                const result = {};
                result.from = fromPort === 'output' ? fromNodeName : [fromNodeName, fromPort];
                result.to = toPort === 'input' ? toNodeName : [toNodeName, toPort];
                return result;
            });
        ...
        requestExport() {
            // Python has the graph state with descriptive names, just signal export
        ...
        handleFileImport(event) {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    this.$emit('import-completed', { success: true, filename: file.name });
                } catch (err) {
                    console.error('Failed to parse pipeline JSON:', err);
                    this.$emit('import-completed', { success: false, error: err.message });
                }
            };
            reader.onerror = () => {
                this.$emit('import-completed', { success: false, error: 'Failed to read file' });
            };
        ...
        loadPreset(presetData) {
            // Load a preset graph configuration (supports multiple formats)
            if (!this.editor || !presetData) return;

            // Suppress incremental events during loading
            ...
            // Legacy format support: separate layout section
            const legacyLayout = presetData.layout || {};

            // First pass: create all nodes
            const nodeIdMap = {};  // Map preset node names to Drawflow IDs
            for (const [nodeName, nodeData] of Object.entries(nodes)) {
                // Get position from:
                // 1. editor sub-attribute (new format)
                // 2. inline x/y (transitional format)
                // 3. legacy layout section
                const editorData = nodeData.editor || {};
                const legacyPos = legacyLayout[nodeName] || {};
                const posX = editorData.x ?? nodeData.x ?? legacyPos.x ?? 200;
                const posY = editorData.y ?? nodeData.y ?? legacyPos.y ?? 200;

                // Determine node type from data structure
                let newId;

                // Explicit class format
                if (nodeData.class) {
                    // Handle special classes
                    if (nodeData.class === 'PipelineSource') {
                        // Explicit format: {"class": "PipelineSource", "type": "SAMPLE", "value": "astronaut"}
                        const imageName = nodeData.value || this.defaultSourceImage;
                        newId = this.addSourceNode(posX, posY, imageName);
                    } else if (nodeData.class === 'PipelineOutput') {
                        newId = this.addOutputNode(posX, posY);
                    } else {
                        // Regular filter
                        newId = this.addFilterNode(nodeData.class, posX, posY);
                        // Merge preset param values with filter metadata
                        if (this.nodes[newId] && nodeData.params) {
                            for (const nodeParam of this.nodes[newId].params) {
                                const presetValue = nodeData.params[nodeParam.name];
                                if (presetValue !== undefined) {
                                    nodeParam.value = presetValue;
                                }
                            }
                        }
                    }
                } else {
                    console.warn(`Unknown node format for "${nodeName}":`, nodeData);
                }

                // Store mapping with the original node name
                if (newId) {
                    nodeIdMap[nodeName] = newId;
                    // Store the preset name for export
                    if (this.nodes[newId]) {
                        this.nodes[newId].presetName = nodeName;
                    }
                }
            }
            ...
            for (const conn of connections) {
                ...
                if ('from' in conn) {
                    // New format
                    const fromPart = conn.from;
                    const toPart = conn.to;
                    if (Array.isArray(fromPart)) {
                        fromNode = fromPart[0];
                        fromPort = fromPart[1];
                    } else {
                        fromNode = fromPart;
                    ...
                    toNode = conn.to_node;
                    fromPort = conn.from_output || 0;
                    toPort = conn.to_input || 0;
                }

                const fromId = nodeIdMap[fromNode];
                const toId = nodeIdMap[toNode];
                if (fromId && toId) {
                    // Convert port names to Drawflow format (output_1, input_1, etc.)
                    let fromOutput, toInput;
                    if (typeof fromPort === 'number') {
                        fromOutput = `output_${fromPort + 1}`;
                    } else {
                        // Named port - look up index from node's output ports
                        const fromNodeData = this.nodes[fromId];
                        const fromIdx = fromNodeData?.outputPorts?.findIndex(p => p.name === fromPort) ?? 0;
                        fromOutput = `output_${Math.max(0, fromIdx) + 1}`;
                    }
                    if (typeof toPort === 'number') {
                        toInput = `input_${toPort + 1}`;
                    } else {
                        // Named port - look up index from node's input ports
                        const toNodeData = this.nodes[toId];
                        const toIdx = toNodeData?.inputPorts?.findIndex(p => p.name === toPort) ?? 0;
                        toInput = `input_${Math.max(0, toIdx) + 1}`;
                    }
                    try {
            ...
            for (const [nodeName, nodeData] of Object.entries(nodes)) {
                // Check for PipelineOutput class, or legacy type === 'output'
                if (nodeData.class === 'PipelineOutput' ||
                    nodeData.output !== undefined ||
                    (nodeData.nodeType || nodeData.type) === 'output') {
                    outputNodeId = nodeIdMap[nodeName];
                    break;
                }
            }
            ...
            // Done loading - re-enable events
            this.loadingPreset = false;

            // Emit ID mapping so Python can sync (name → Drawflow ID)
            this.$emit('preset-loaded', { idToName: nodeIdMap });
        }
    }
};
