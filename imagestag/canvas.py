...
from .pos2d import Pos2DTypes, Pos2D
...
def _color_to_format(color: ColorTypes, pixel_format: PixelFormat) -> tuple | int:
    """
    Convert a color to the appropriate format for the given pixel format.

    For grayscale images, allows passing a single int (0-255) directly.

    :param color: The color value
    :param pixel_format: The target pixel format
    :return: The color in the appropriate format for PIL drawing
    """
    # For grayscale, allow passing int directly
    ...
    # Otherwise, convert through Color class
    return Color(color).to_format(pixel_format)
...
class Canvas:
    ...
    Example usage::
        ...
        # Create a grayscale mask for layer composition
        canvas = Canvas(size=(200, 150), pixel_format="L")
        canvas.circle((100, 75), radius=70, color=255)  # White circle
        mask = canvas.to_image()

        # Create an RGBA canvas for overlay
        canvas = Canvas(size=(400, 300), pixel_format="RGBA")
        canvas.rect((10, 10), (100, 50), color=Colors.RED)
        canvas.text((50, 100), "Hello!", color=Colors.WHITE)
        overlay = canvas.to_image()
    """
    ...
    def __init__(
        ...
        :param pixel_format: The image format, currently only RGB, RGBA and L (grayscale)
        """
        ...
        """
        self.target_image: PIL.Image.Image
        ...
        if target_image is not None:
            ...
            # Use _color_to_format to properly handle grayscale colors
            color = _color_to_format(default_color, pixel_format)
    ...
    def to_image(self) -> Image:
        ...
        will not be a copy and point to the current PIL image handle as well.
    ...
    def add_offset_shift(self, offset: tuple[float, float]) -> Canvas:
        """
    ...
    def clip(self, offset: tuple[float, float], size: tuple[float, float]) -> Canvas:
        """
    ...
    def get_default_font(
        self, size_factor: float = 1.0, size: float | None = None, flags: set[str] | None = None
    ) -> Font:
        """
    ...
    def draw_image(self, image: Image, pos: Pos2DTypes, auto_blend: bool = True) -> Canvas:
        """
    ...
    def rect(
        ...
        if color is not None:
            color = _color_to_format(color, pixel_format)
        if outline_color is not None:
            outline_color = _color_to_format(outline_color, pixel_format)
    ...
    def rectangle_list(
        ...
        :param colors: The list of colors (has to match the length of rectangles)
        :param single_color: The rectangle color (if all rectangles have the same color)
    ...
    def polygon(
        ...
        :param coords: A list of coordinates defining the polygon's vertices
        :param color: The inner color
        :param outline_color: The outline color
        :param outline_width: The outline's width
        :return: Self
    ...
    def line(
        ...
        :param curved_joints: Defines if the connections shall be smoothed,
            recommended for very thick lines
        :return: Self
        """
        if isinstance(coords, np.ndarray):
            coords = coords.astype(float).tolist()
    ...
    def circle(
        ...
        self.image_draw.ellipse(
            xy=coords, fill=color, outline=outline_color, width=int(outline_width)
        )
        return self
    ...
    def text(
        ...
        :param stroke_width: The stroke width in pixels.
