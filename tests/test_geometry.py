"""
Tests for Size2D, Pos2D and Bounding2D classes
"""

import pytest

from imagestag import Size2D, Pos2D, Bounding2D
...
def test_size2d_none():
    """
    Tests Size2D with None value
    """
    size = Size2D(None)
...
def test_bounding_int_coord_tuple():
    """
    Tests integer coordinate tuple conversion
    """
    bounding = Bounding2D((45.4, 50.6, 100.1, 100.9))
    int_coords = bounding.to_int_coord_tuple()
    assert int_coords == (45, 51, 100, 101)
    assert all(isinstance(c, int) for c in int_coords)


def test_bounding_int_coord_size_tuple():
    """
    Tests integer coordinate size tuple conversion
    """
    bounding = Bounding2D((45.4, 50.6, 100.1, 100.9))
    int_coords = bounding.to_int_coord_size_tuple()
    assert all(isinstance(c, int) for c in int_coords)
