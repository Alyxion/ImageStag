"""Tests for brush and eraser tools with different layer offsets - Playwright version.
...
- For circles/round brushes: area = pi*r^2 where r = size/2
...
class TestBrushTool:
    ...
    async def test_brush_horizontal_stroke(self, helpers: TestHelpers):
    ...
    async def test_brush_vertical_stroke(self, helpers: TestHelpers):
    ...
    async def test_brush_diagonal_stroke(self, helpers: TestHelpers):
        """Test diagonal brush stroke produces expected pixel count."""
        await helpers.new_document(200, 200)

        brush_size = 6
        # Diagonal from (30, 30) to (170, 170) = sqrt(140^2 + 140^2) = 198 pixels
    ...
    async def test_brush_dot_produces_circle(self, helpers: TestHelpers):
    ...
    async def test_brush_outside_offset_layer_no_pixels(self, helpers: TestHelpers):
    ...
    async def test_brush_size_scaling(self, helpers: TestHelpers):
        ...
        await helpers.tools.brush_dot(225, 100, color='#00FF00', size=20)
...
class TestEraserTool:
    ...
    async def test_eraser_removes_expected_pixels(self, helpers: TestHelpers):
        ...
        await helpers.tools.eraser_stroke([(50, 100), (150, 100)], size=eraser_size)

        remaining_red = await helpers.pixels.count_pixels_with_color((255, 0, 0, 255), tolerance=10)
    ...
    async def test_eraser_on_offset_layer(self, helpers: TestHelpers):
        ...
        await helpers.tools.eraser_stroke([(150, 200), (250, 200)], size=eraser_size)
    ...
    async def test_eraser_outside_offset_layer_no_effect(self, helpers: TestHelpers):
        ...
        await helpers.tools.eraser_stroke([(10, 10), (100, 100)], size=20)
    ...
    async def test_eraser_crossing_layer_edge(self, helpers: TestHelpers):
        ...
        await helpers.tools.eraser_stroke([(50, 150), (150, 150)], size=eraser_size)
...
class TestUndoRedo:
    ...
    async def test_undo_restores_exact_pixel_count(self, helpers: TestHelpers):
        """Test that undo restores exact original pixel count."""
        await helpers.new_document(200, 200)

        initial_non_transparent = await helpers.pixels.count_non_transparent_pixels()
    ...
    async def test_redo_restores_exact_pixel_count(self, helpers: TestHelpers):
    ...
    async def test_undo_eraser_restores_exact_count(self, helpers: TestHelpers):
        """Test that undo after eraser restores exact pixel count."""
        await helpers.new_document(200, 200)

        await helpers.layers.fill_layer_with_color('#00FF00')
        after_fill = await helpers.pixels.count_non_transparent_pixels()
        assert after_fill == 200 * 200

        await helpers.tools.eraser_stroke([(50, 50), (150, 150)], size=20)
        after_erase = await helpers.pixels.count_non_transparent_pixels()
